...
def create_mcp_server(store: ConditionStore) -> FastMCP:
    ...
    def query_conditions(text: str = "", active_only: bool = False, include_admin: bool = False, limit: int = 20) -> dict:
        ...
        Designed for RAG use: returns immediately with the best available data,
        even if background ingestion is still running.  Check `partial` in the
        response to know whether all data has been loaded.

        Args:
            text:          Free-text filter.  Matches display name, SNOMED code,
                        or ICD-10 code.  Empty string returns all conditions.
    ...
    def correct_condition(concept: str) -> dict:
        ...
        Use this when the user says they do not have a condition, or that a record
        was entered in error.

        Args:
            concept: Name or code to retract.  Examples:
        ...
        The retraction is applied immediately and persists for the lifetime of the
        store.  Future query_conditions calls will exclude retracted concepts unless
        include_retracted=True is explicitly passed to the store's query() method.
        """
    ...
    def store_status() -> dict:
        ...
        Returns:
            ...
            retracted_groups: int,
            admin_groups: int,
            ingestion_log: [ { batch, timestamp, added, skipped, errors,
                                elapsed_ms, total_records_now, total_groups_now } ]
            }
        """
